# 头部导航用户信息显示功能开发规范

> 本文档详细说明项目中头部左侧导航栏用户信息显示功能的实现方式、技术架构和开发规范

## 📋 目录

- [功能概述](#功能概述)
- [技术架构](#技术架构)
- [实现细节](#实现细节)
- [代码结构](#代码结构)
- [开发规范](#开发规范)
- [扩展指南](#扩展指南)
- [常见问题](#常见问题)

---

## 功能概述

### 功能描述

头部左侧导航栏用户信息显示功能位于应用主布局的侧边栏（`a-layout-sider`）底部，主要包含：

1. **用户名称显示**：显示当前登录用户的真实姓名或用户名
2. **登出功能**：提供登出按钮，支持用户安全退出系统

### 功能特点

- ✅ **响应式设计**：支持侧边栏折叠/展开状态
- ✅ **状态持久化**：用户信息自动保存到 localStorage
- ✅ **权限集成**：与权限系统无缝集成
- ✅ **类型安全**：完整的 TypeScript 类型定义
- ✅ **状态管理**：基于 Pinia 的集中式状态管理

---

## 技术架构

### 技术栈

| 技术 | 版本 | 用途 |
|------|------|------|
| **Vue 3** | 3.5+ | 前端框架 |
| **TypeScript** | 5.9+ | 类型系统 |
| **Pinia** | 3.0+ | 状态管理 |
| **Ant Design Vue** | 4.2+ | UI 组件库 |
| **Vue Router** | 4.6+ | 路由管理 |

### 架构图

```
┌─────────────────────────────────────────────────────────┐
│                    App.vue (主布局)                      │
│  ┌───────────────────────────────────────────────────┐  │
│  │         a-layout-sider (侧边栏)                    │  │
│  │  ┌─────────────────────────────────────────────┐  │  │
│  │  │  Logo 区域                                   │  │  │
│  │  └─────────────────────────────────────────────┘  │  │
│  │  ┌─────────────────────────────────────────────┐  │  │
│  │  │  Menu 菜单区域                               │  │  │
│  │  └─────────────────────────────────────────────┘  │  │
│  │  ┌─────────────────────────────────────────────┐  │  │
│  │  │  user-info (用户信息区域) ← 本文档重点       │  │  │
│  │  │  ├─ user-name (用户名显示)                   │  │  │
│  │  │  └─ 登出按钮                                 │  │  │
│  │  └─────────────────────────────────────────────┘  │  │
│  └───────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│              useUserStore (Pinia Store)                  │
│  ├─ userInfo: UserInfo | null                           │
│  ├─ token: string                                       │
│  ├─ userName: computed (计算属性)                       │
│  ├─ login()                                             │
│  ├─ logout()                                            │
│  └─ getCurrentUser()                                    │
└─────────────────────────────────────────────────────────┘
                          ↓
┌─────────────────────────────────────────────────────────┐
│                  userApi (API 层)                        │
│  ├─ login()                                             │
│  ├─ getCurrentUser()                                    │
│  └─ logout()                                            │
└─────────────────────────────────────────────────────────┘
```

---

## 实现细节

### 1. 组件结构 (App.vue)

#### 模板部分

```vue
<a-layout-sider v-model:collapsed="collapsed" collapsible>
  <!-- Logo 区域 -->
  <div class="logo">
    <h2 style="color: white; margin: 0; font-size: 18px">Vue App</h2>
  </div>
  
  <!-- 菜单区域 -->
  <a-menu ...>
    <!-- 菜单项 -->
  </a-menu>
  
  <!-- 用户信息区域 -->
  <div class="user-info">
    <div class="user-name">{{ userName }}</div>
    <a-button 
      type="text" 
      size="small" 
      @click="handleLogout" 
      style="color: white; width: 100%"
    >
      登出
    </a-button>
  </div>
</a-layout-sider>
```

#### 脚本部分

```typescript
<script lang="ts" setup>
import { ref, computed } from 'vue'
import { useRouter, useRoute } from 'vue-router'
import { useUserStore } from '@/store/modules/user'
import { usePermissionStore } from '@/store/modules/permission'

const router = useRouter()
const route = useRoute()
const userStore = useUserStore()
const permissionStore = usePermissionStore()

// 用户名计算属性
const userName = computed(() => {
  return userStore.userName || '用户'
})

// 登出处理
async function handleLogout() {
  await userStore.logout()
  permissionStore.resetPermission()
  resetRouteAdded()
  router.push('/login')
}
</script>
```

#### 样式部分

```css
<style scoped>
.user-info {
  position: absolute;
  bottom: 0;
  width: 100%;
  padding: 16px;
  border-top: 1px solid rgba(255, 255, 255, 0.1);
}

.user-name {
  color: white;
  font-size: 14px;
  margin-bottom: 8px;
  text-align: center;
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
}
</style>
```

### 2. 状态管理 (useUserStore)

#### Store 定义

```typescript
// src/store/modules/user.ts
import { defineStore } from 'pinia'
import { ref, computed } from 'vue'
import { userApi } from '@/api/user'
import type { UserInfo, LoginRequest } from '@/api/types'

export const useUserStore = defineStore(
  'user',
  () => {
    // 状态
    const token = ref<string>('')
    const userInfo = ref<UserInfo | null>(null)
    const loading = ref(false)

    // 计算属性：用户名
    const userName = computed(() => {
      return userInfo.value?.realName || userInfo.value?.username || ''
    })

    // 计算属性：用户角色
    const userRoles = computed(() => {
      return userInfo.value?.roles || []
    })

    // 计算属性：用户权限
    const userPermissions = computed(() => {
      return userInfo.value?.permissions || []
    })

    // 登录方法
    async function login(loginData: LoginRequest) {
      loading.value = true
      try {
        const response = await userApi.login(loginData)
        token.value = response.data.token
        userInfo.value = response.data.user
        return response.data
      } finally {
        loading.value = false
      }
    }

    // 获取当前用户信息
    async function getCurrentUser() {
      loading.value = true
      try {
        const response = await userApi.getCurrentUser()
        userInfo.value = response.data
        return response.data
      } finally {
        loading.value = false
      }
    }

    // 登出方法
    async function logout() {
      loading.value = true
      try {
        await userApi.logout()
      } catch (error) {
        console.error('登出失败:', error)
      } finally {
        token.value = ''
        userInfo.value = null
        loading.value = false
      }
    }

    return {
      // 状态
      token,
      userInfo,
      loading,
      // 计算属性
      userName,
      userRoles,
      userPermissions,
      // 方法
      login,
      getCurrentUser,
      logout
    }
  },
  {
    // 持久化配置
    persist: {
      enabled: true,
      strategies: [
        {
          key: 'user_store',
          storage: localStorage,
          paths: ['token', 'userInfo']
        }
      ]
    }
  }
)
```

### 3. API 层 (userApi)

```typescript
// src/api/user.ts
import { http } from './http'
import type { LoginRequest, LoginResponse, UserInfo } from './types'

export const userApi = {
  /**
   * 登录
   */
  login(params: LoginRequest) {
    return http.post<LoginResponse>('/user/login', params)
  },

  /**
   * 获取当前用户信息
   */
  getCurrentUser() {
    return http.get<UserInfo>('/user/info')
  },

  /**
   * 登出
   */
  logout() {
    return http.post('/user/logout')
  }
}
```

### 4. 类型定义

```typescript
// src/api/types.ts

/**
 * 用户信息接口
 */
export interface UserInfo {
  id: string
  username: string
  realName: string
  email: string
  avatar: string
  roles: string[]
  permissions: string[]
}

/**
 * 登录请求参数
 */
export interface LoginRequest {
  username: string
  password: string
}

/**
 * 登录响应数据
 */
export interface LoginResponse {
  token: string
  user: UserInfo
}
```

---

## 代码结构

### 文件组织

```
src/
├── App.vue                    # 主布局组件（用户信息显示区域）
├── api/
│   ├── user.ts               # 用户相关 API
│   └── types.ts              # API 类型定义
├── store/
│   └── modules/
│       └── user.ts           # 用户状态管理
└── router/
    └── index.ts              # 路由配置
```

### 关键文件说明

| 文件路径 | 职责 | 关键内容 |
|---------|------|---------|
| `src/App.vue` | 主布局组件 | 用户信息显示 UI、登出处理 |
| `src/store/modules/user.ts` | 用户状态管理 | 用户信息状态、登录/登出逻辑 |
| `src/api/user.ts` | API 接口 | 用户相关 HTTP 请求 |
| `src/api/types.ts` | 类型定义 | UserInfo、LoginRequest 等类型 |

---

## 开发规范

### 1. 组件开发规范

#### ✅ 推荐做法

1. **使用 Composition API**
   ```vue
   <script lang="ts" setup>
   // 使用 <script setup> 语法
   </script>
   ```

2. **使用计算属性获取用户信息**
   ```typescript
   const userName = computed(() => {
     return userStore.userName || '用户'
   })
   ```

3. **使用 TypeScript 类型**
   ```typescript
   import type { UserInfo } from '@/api/types'
   ```

4. **错误处理**
   ```typescript
   async function handleLogout() {
     try {
       await userStore.logout()
     } catch (error) {
       console.error('登出失败:', error)
       // 错误提示
     }
   }
   ```

#### ❌ 避免做法

1. **不要直接修改 Store 状态**
   ```typescript
   // ❌ 错误
   userStore.userInfo = newUserInfo
   
   // ✅ 正确
   userStore.setUserInfo(newUserInfo)
   ```

2. **不要在组件中直接调用 API**
   ```typescript
   // ❌ 错误
   const userInfo = await userApi.getCurrentUser()
   
   // ✅ 正确
   const userInfo = await userStore.getCurrentUser()
   ```

3. **不要硬编码用户信息**
   ```typescript
   // ❌ 错误
   const userName = '管理员'
   
   // ✅ 正确
   const userName = computed(() => userStore.userName)
   ```

### 2. Store 开发规范

#### ✅ 推荐做法

1. **使用计算属性派生状态**
   ```typescript
   const userName = computed(() => {
     return userInfo.value?.realName || userInfo.value?.username || ''
   })
   ```

2. **使用持久化插件**
   ```typescript
   {
     persist: {
       enabled: true,
       strategies: [
         {
           key: 'user_store',
           storage: localStorage,
           paths: ['token', 'userInfo']
         }
       ]
     }
   }
   ```

3. **统一错误处理**
   ```typescript
   async function logout() {
     loading.value = true
     try {
       await userApi.logout()
     } catch (error) {
       console.error('登出失败:', error)
     } finally {
       // 无论成功失败都清除本地数据
       token.value = ''
       userInfo.value = null
       loading.value = false
     }
   }
   ```

### 3. API 开发规范

#### ✅ 推荐做法

1. **使用类型定义**
   ```typescript
   login(params: LoginRequest): Promise<ApiResponse<LoginResponse>>
   ```

2. **统一错误处理**
   ```typescript
   // 在 http 拦截器中统一处理
   ```

3. **使用 JSDoc 注释**
   ```typescript
   /**
    * 登录
    * @param params 登录参数
    * @returns 登录响应
    */
   login(params: LoginRequest) {
     return http.post<LoginResponse>('/user/login', params)
   }
   ```

### 4. 样式开发规范

#### ✅ 推荐做法

1. **使用 scoped 样式**
   ```vue
   <style scoped>
   .user-info {
     /* 样式 */
   }
   </style>
   ```

2. **响应式设计**
   ```css
   .user-name {
     overflow: hidden;
     text-overflow: ellipsis;
     white-space: nowrap;
   }
   ```

3. **使用 CSS 变量（如需要主题切换）**
   ```css
   .user-name {
     color: var(--text-color);
   }
   ```

---

## 扩展指南

### 1. 添加用户头像显示

#### 修改 App.vue

```vue
<div class="user-info">
  <!-- 添加头像 -->
  <a-avatar 
    :src="userStore.userInfo?.avatar" 
    :size="40"
    style="margin-bottom: 8px"
  >
    <template #icon>
      <UserOutlined />
    </template>
  </a-avatar>
  
  <div class="user-name">{{ userName }}</div>
  <a-button 
    type="text" 
    size="small" 
    @click="handleLogout" 
    style="color: white; width: 100%"
  >
    登出
  </a-button>
</div>
```

#### 修改样式

```css
.user-info {
  position: absolute;
  bottom: 0;
  width: 100%;
  padding: 16px;
  border-top: 1px solid rgba(255, 255, 255, 0.1);
  display: flex;
  flex-direction: column;
  align-items: center;
}
```

### 2. 添加用户下拉菜单

#### 使用 Ant Design Vue 的 Dropdown

```vue
<a-dropdown placement="top">
  <div class="user-info-trigger">
    <a-avatar :src="userStore.userInfo?.avatar" :size="32" />
    <span class="user-name">{{ userName }}</span>
  </div>
  <template #overlay>
    <a-menu>
      <a-menu-item key="profile">
        <UserOutlined />
        个人中心
      </a-menu-item>
      <a-menu-item key="settings">
        <SettingOutlined />
        设置
      </a-menu-item>
      <a-menu-divider />
      <a-menu-item key="logout" @click="handleLogout">
        <LogoutOutlined />
        登出
      </a-menu-item>
    </a-menu>
  </template>
</a-dropdown>
```

### 3. 添加用户状态指示器

```vue
<div class="user-info">
  <div class="user-status">
    <a-avatar :src="userStore.userInfo?.avatar" :size="32" />
    <span class="status-dot" :class="{ online: isOnline }"></span>
  </div>
  <div class="user-name">{{ userName }}</div>
</div>
```

```css
.user-status {
  position: relative;
  margin-bottom: 8px;
}

.status-dot {
  position: absolute;
  bottom: 0;
  right: 0;
  width: 10px;
  height: 10px;
  border-radius: 50%;
  background: #ccc;
  border: 2px solid white;
}

.status-dot.online {
  background: #52c41a;
}
```

### 4. 添加用户角色显示

```vue
<div class="user-info">
  <div class="user-name">{{ userName }}</div>
  <div class="user-roles">
    <a-tag 
      v-for="role in userStore.userRoles" 
      :key="role"
      size="small"
      color="blue"
    >
      {{ role }}
    </a-tag>
  </div>
  <a-button type="text" size="small" @click="handleLogout">
    登出
  </a-button>
</div>
```

---

## 常见问题

### Q1: 用户信息不显示？

**原因分析**：
1. Store 中用户信息为空
2. 计算属性逻辑错误
3. 持久化数据未正确恢复

**解决方案**：
```typescript
// 检查 Store 状态
console.log('userInfo:', userStore.userInfo)
console.log('userName:', userStore.userName)

// 确保在应用启动时恢复用户信息
onMounted(async () => {
  if (userStore.token && !userStore.userInfo) {
    await userStore.getCurrentUser()
  }
})
```

### Q2: 登出后用户信息仍然显示？

**原因分析**：
1. Store 状态未正确清除
2. 路由跳转失败
3. 持久化数据未清除

**解决方案**：
```typescript
async function handleLogout() {
  try {
    await userStore.logout()
    // 清除持久化数据
    localStorage.removeItem('user_store')
    // 重置权限
    permissionStore.resetPermission()
    // 跳转到登录页
    router.push('/login')
  } catch (error) {
    console.error('登出失败:', error)
  }
}
```

### Q3: 侧边栏折叠时用户信息显示异常？

**原因分析**：
1. 固定定位导致布局问题
2. 宽度计算错误

**解决方案**：
```css
.user-info {
  position: absolute;
  bottom: 0;
  width: 100%;
  padding: 16px;
  box-sizing: border-box;
}

/* 折叠状态下的样式 */
.ant-layout-sider-collapsed .user-info {
  padding: 8px;
}

.ant-layout-sider-collapsed .user-name {
  display: none; /* 或使用图标代替 */
}
```

### Q4: 如何实现用户信息实时更新？

**解决方案**：
```typescript
// 在 Store 中添加更新方法
async function updateUserInfo() {
  try {
    const response = await userApi.getCurrentUser()
    userInfo.value = response.data
  } catch (error) {
    console.error('更新用户信息失败:', error)
  }
}

// 在组件中定期刷新
onMounted(() => {
  // 每 5 分钟刷新一次用户信息
  const timer = setInterval(() => {
    userStore.updateUserInfo()
  }, 5 * 60 * 1000)
  
  onUnmounted(() => {
    clearInterval(timer)
  })
})
```

---

## 最佳实践总结

### ✅ 推荐做法

1. **使用 Pinia Store 管理用户状态**
2. **使用计算属性派生显示数据**
3. **实现状态持久化**
4. **统一错误处理**
5. **使用 TypeScript 确保类型安全**
6. **响应式设计，适配不同屏幕尺寸**

### ❌ 避免做法

1. **不要在组件中直接操作 API**
2. **不要硬编码用户信息**
3. **不要忽略错误处理**
4. **不要忘记清除敏感数据**

---

## 参考资料

- [Vue 3 官方文档](https://vuejs.org/)
- [Pinia 官方文档](https://pinia.vuejs.org/)
- [Ant Design Vue 官方文档](https://antdv.com/)
- [Vue Router 官方文档](https://router.vuejs.org/)

---

**文档版本**: v1.0  
**最后更新**: 2025-01-15  
**维护者**: 开发团队

